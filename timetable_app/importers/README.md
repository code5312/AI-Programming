...
> 가천대가 아닌 다른 학교/학기 데이터를 쓰고 싶은데 여기에 맞는 임포터를 만들 시간이 없다면, 굳이 여기부터 시작할 필요 없다. 웹 UI의 **CSV 업로드**(`/upload`)로 `courses.csv`와 같은 형식의 파일을 직접 올리면 된다 — 이 디렉터리는 "매 학기 반복되는 수작업을 자동화"하는 용도고, 업로드는 "한 번 써보고 싶다"는 용도에 가깝다.
